...
Route::post('reports/officecontribution', function(){
            ...
            if(Input::get('member_id') === 'all'){
                $contributions = Contribution::where('is_void',false)->whereBetween('date', array($from, $to))->where('type','=','credit')->get();
                $pdf = PDF::loadView('pdf.officeallcontreport', compact('members','contributions','from','to', 'organization'))
                ->setPaper('a4')->setOrientation('potrait');
                return $pdf->stream('Contribution.pdf');
            }else{
                $contributions = Contribution::where('member_id',Input::get('member_id'))->where('is_void',false)
                ->whereBetween('date', array($from, $to))->get();
                $pdf = PDF::loadView('pdf.officecontreport', compact('member','contributions','from','to', 'organization'))->setPaper('a4')
